...
# ── Combined JS output (both corpora in one file, colocated with the tool) ────
COMBINED_JS = PROJECT_ROOT / "tools" / "metrics_explorer" / "metrics_explorer_data.js"
COMBINED_JS_CODER = (
    PROJECT_ROOT / "tools" / "gbls_lit_coder" / "public" / "metrics_explorer"
...
def main() -> None:
    ...
    if COMBINED_JS_CODER.parent.exists():
        shutil.copy2(COMBINED_JS, COMBINED_JS_CODER)
        print(f"  Copied → {COMBINED_JS_CODER.relative_to(PROJECT_ROOT)}")
    print()
